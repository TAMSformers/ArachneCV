...
void find_noodles(cv::Mat frame, cv::Mat warp, cv::Mat depth, std::vector<Target> &r_targets, double effective_height, double depth_correction, int hfov, int vfov)
{
  ...
  /* light green bounds for inRange (in HSV color space) */
  int lower_bound[3] = {35, 80, 80};
  int upper_bound[3] = {60, 255, 255};
  ...
  add_color_fields_as_targets(r_targets, centers, "noodle", orientations, angles);

  int debug_color[3] = {255, 0, 0};
